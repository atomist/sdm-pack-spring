...
class MavenSourceDeployer implements Deployer<ManagedDeploymentTargetInfo> {
    ...
    public async deploy(da: DeployableArtifact,
        ...
        const port = await managedMavenDeployments.findPort(ti.managedDeploymentKey, LookupStrategy.branch,
            this.opts.baseUrl);
        logger.info("MavenSourceDeployer: Deploying app [%j],branch=%s on port [%d] for team %s", id,
            ti.managedDeploymentKey.branch, port, team);
