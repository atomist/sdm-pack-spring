...
export const XmlTagWithSimpleValueGrammar = {
    ...
    _l2: "</",
    _close: ELEMENT_NAME,
    _ok: (ctx: any) => ctx._close === ctx.name,
    _r2: ">",
};
...
export const GavGrammar = Microgrammar.fromDefinitions<{ gav: VersionedArtifact }>({
    ...
    _valid: (ctx: any) =>
        ctx.tags.filter((t: any) => t.name === "groupId").length > 0 &&
        ctx.tags.filter((t: any) => t.name === "artifactId").length > 0,
    gav: (ctx: any) => {
        const group = ctx.tags.filter((tag: any) => tag.name === "groupId")[0].value;
        const artifact = ctx.tags.filter((tag: any) => tag.name === "artifactId")[0].value;
        const versions = ctx.tags.filter((tag: any) => tag.name === "version");
...
function artifactContainerGrammar(containerElementName: string, group?: string, artifact?: string) {
    return Microgrammar.fromDefinitions<ArtifactContainer>({
        ...
        _hasDesiredGroup: (ctx: any) => !!artifact ? ctx._gav.gav.group === group : true,
        _hasDesiredArtifact: (ctx: any) => !!artifact ? ctx._gav.gav.artifact === artifact : true,
        gav: (ctx: any) => ctx._gav.gav,
        ...
        version: (ctx: any) => ctx._gav.tags.find((t: any) => t.name === "version"),
    });
}
